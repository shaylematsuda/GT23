...
#issue with Tube 436. frag 92 (acer) tube 436 was written on data sheet. But it was not erased from 1291-8 (pcomp). for NOW keep frag 92 but check NMDS to confirm species. potentially trash. 12/19/25 checked NMDS nat ACER, trash incase of contam
md <- md %>%
  filter(!(Tube == 436 & Tank.datasheet == "HI_Control_2"))
md <- md %>%
  filter(!(Tube == 436 & Tank.datasheet == "NWQ4"))
...
# look at 'extras', these were extra frags of same genos duped in controls only. drop frag 58, 86, and 99
Bac.seq<-subset_samples(Bac.seq, Frag.ID!="58")
Bac.seq<-subset_samples(Bac.seq, Frag.ID!="86")
Bac.seq<-subset_samples(Bac.seq, Frag.ID!="99")
Bac.seq
```
Save file
```{r}
